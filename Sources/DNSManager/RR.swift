...
public struct DNSRR {
    ...
    public static func deserialize(data: [UInt8]) -> DNSRR {
        ...
        while i > 0 {
            let (name, offset) = self.deserializeName(data: data, startAt: pos)
        ...
        i = anCount
        while i > 0 {
            var (name, offset) = self.deserializeName(data: data, startAt: pos)
            ...
            if typ == 1 && rlen == 4 {
                ...
                (rdata, offset) = self.deserializeName(data: data, startAt: pos)
                pos += offset
            } else if typ == 16 {
                var txts: [String] = []
                while pos < alen {
    ...
    private static func deserializeName(data: [UInt8], startAt: Int) -> (String, Int) {
        ...
        while true {
            ...
            if data[i] & 0xc0 == 0xc0 {
                let (s, _) = self.deserializeName(data: data, startAt: Int(data[i] & 0x3f << 8)+Int(data[i+1]))
